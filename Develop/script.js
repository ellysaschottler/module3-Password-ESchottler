...
function getValidCharacterNumber() {
  ...
  if (characterNumber < 8 || characterNumber > 128) {
    alert("This is an invalid number of characters.");
    return getValidCharacterNumber();
  } else if (characterNumber >= 8 && characterNumber <= 128) {
    alert(
      `Your password will be ${characterNumber} characters long.  Please answer the next questions to determine the character types in your password.`
    );
  }
...
function getValidCharacters() {
  ...
  return (
    lowercaseLetters + uppercaseLetters + numericCharacters + specialCharacters
  );
}
